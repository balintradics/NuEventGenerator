...
double mC = 12.0*au;
double mProton = 1.007276466879*au; // 
double mNeutron = 1.00866491588*au; // 
...
void generatePS_NucleonFermiMotion(){
  ...
  // And TGenPhaseSpace gives back the final state 4-momenta in the Lab frame.
  //
  // Because the initial state nucleon in a nucleus is not at rest we generate a random 4-mom
  // for the initial nucleon and then Lorentz-boost the neutrino beam into that frame. 
  // We evaluate the LL-Smith diff. cross section in that frame and finally boost
  // back the final state particles to the Lab frame.
  //
  // Next, we will use the Neumann acceptance-rejection method to decide which
  // throws of of 4-momentum in the final state from TGenPhaseSpace to accept according to the
  // LL-Smith diff. cross section function.
  ...
  for (Int_t n=0;n<Nevents;n++) {
    ...
    Double_t masses[2] = { mProton , mMuon};
    ...
    if(accept){
      ...
      hPt_Muon->Fill(pt_muon, weight*xsec); 
      hPz_Muon->Fill(pz_muon, weight*xsec);
      hPhi_Muon->Fill(phi_muon, weight*xsec);
      hE_Muon->Fill(pMuon->E(), weight*xsec);
      hQ2->Fill(Q2Lab, weight*xsec);


      // Try to reconstruct the neutrino energy?
      double Eb = 0; // Eb should be included later...
      double EnuReco =0.5*( mProton*mProton - TMath::Power((mNeutron - Eb), 2) + mMuon*mMuon + 2*(mNeutron - Eb)*pMuon->E())/ (mNeutron - Eb - pMuon->E()+ p_muon*TMath::Cos(Theta_Muon_Nu));
